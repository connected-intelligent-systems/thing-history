...
/* Create the flux query to return the last value of a thing property 
 * @param {string} thingId - the thing id
 * @param {string} propertyName - the name of the property
 */
function createLastQuery (thingId, propertyName, options = {}) {
  const { start = "-1d" } = options
  return flux`from(bucket: ${fluxString(Bucket)})
...
/* Run a influxdb query async in memory
 * @param {string} query - the query
 */
async function queryLines (query) {
...
exports = module.exports = (req, res, next) => {
  req.influxDB = {
    ...
    last: async(thingId, propertyName, options) => {
      try {
        console.log(createLastQuery(thingId, propertyName, options).fluxValue)
        const results = await queryLines(createLastQuery(thingId, propertyName, options), req, res)
        res.json({
